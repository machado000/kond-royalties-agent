...
-- BRONZE__GA4_PAGEVIEW_DAILY_INCREMENTAL
...
CREATE TABLE IF NOT EXISTS `g-analytics-487213.silver_us.ga4_pageviews`
...
FROM g-analytics-487213.bronze_us.ga4_events_pageview
WHERE FALSE
GROUP BY ALL;

-- Rebuild only recent partitions to keep the load idempotent and absorb late arrivals.
DELETE FROM `g-analytics-487213.silver_us.ga4_pageviews`
WHERE event_date BETWEEN start_date AND end_date;

INSERT INTO `g-analytics-487213.silver_us.ga4_pageviews`
  (
  event_date,
  event_name,
  page_location,
